...
# from models.invertible_resnet.spectral_norm_conv_inplace import spectral_norm_conv
# from models.invertible_resnet.spectral_norm_fc import spectral_norm_fc
# from models.invertible_resnet.matrix_utils import exact_matrix_logarithm_trace, power_series_matrix_logarithm_trace
...
class conv_iResNet(nn.Module):
    ...
    def classifier(self, z):
        out = F.leaky_relu(self.bn1(z), negative_slope=0.2)
        # out = F.elu(self.bn1(z))
        out = F.avg_pool2d(out, out.size(2))
        out = out.view(out.size(0), out.size(1))
        return self.logits(out)
    ...
    def set_num_terms(self, n_terms):
        for block in self.stack:
            ...
            block.numTraceSamples = n_terms


def iresnet(**kwargs):
    return {'backbone': conv_iResNet(
                            nBlocks=[7, 7, 7], nStrides=[1, 2, 2],
                            nChannels=[32, 64, 128], nClasses=512,
                            init_ds=1,
                            inj_pad=13,
                            in_shape=(3, 32, 32),
                            coeff=0.9,
