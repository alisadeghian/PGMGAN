...
def build_models(config):
    ...
    # Build models
    if config['generator']['name'] == 'biggan':
        generator = Generator(**config['biggan'])
        discriminator = Discriminator(**config['biggan'])
    else:
...
def build_optimizers(generator, discriminator, config):
    ...
    if optimizer == 'rmsprop':
        ...
        try:
            epsilon = config['training']['adam_eps']
        except:
            epsilon = 1e-8
        g_optimizer = optim.Adam(g_params, lr=lr_g, betas=(beta1, beta2), eps=epsilon)
        d_optimizer = optim.Adam(d_params, lr=lr_d, betas=(beta1, beta2), eps=epsilon)
    elif optimizer == 'sgd':
